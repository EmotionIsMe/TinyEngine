...
public:
		...
		: Layer("Example"), m_Camera(-1.6f, 1.6f, -0.9f, 0.9f), m_CameraPosition(0.0f)
	{
		...
		m_Shader.reset(new TinyEngine::Shader(vertexSrc, fragmentSrc));
		...
		m_BlueShader.reset(new TinyEngine::Shader(blueShaderVertexSrc, blueShaderFragmentSrc));
	}
	
	void OnUpdate(TinyEngine::Timestep ts) override
	{
		if (TinyEngine::Input::IsKeyPressed(TE_KEY_LEFT))
			m_CameraPosition.x -= m_CameraMoveSpeed * ts;
		else if (TinyEngine::Input::IsKeyPressed(TE_KEY_RIGHT))
			m_CameraPosition.x += m_CameraMoveSpeed * ts;

		if (TinyEngine::Input::IsKeyPressed(TE_KEY_UP))
			m_CameraPosition.y += m_CameraMoveSpeed * ts;
		else if (TinyEngine::Input::IsKeyPressed(TE_KEY_DOWN))
			m_CameraPosition.y -= m_CameraMoveSpeed * ts;

		if (TinyEngine::Input::IsKeyPressed(TE_KEY_A))
			m_CameraRotation += m_CameraRotationSpeed * ts;
		if (TinyEngine::Input::IsKeyPressed(TE_KEY_D))
			m_CameraRotation -= m_CameraRotationSpeed * ts;
		...
		for (int y = 0; y < 20; y++)
		{
			for (int x = 0; x < 20; x++)
			{
				...
				TinyEngine::Renderer::Submit(m_BlueShader, m_SquareVA, transform);
			}
		}
	...
	private:
		...
		std::shared_ptr<TinyEngine::Shader> m_BlueShader;
