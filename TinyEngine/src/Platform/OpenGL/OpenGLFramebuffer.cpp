...
namespace TinyEngine {
	...
	namespace Utils {
		...
		static GLenum TinyFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			TE_CORE_ASSERT(m_ColorAttachments.size() <= 4);
	...
	{
		TE_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::TinyFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
}