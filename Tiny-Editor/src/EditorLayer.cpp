...
#include "TinyEngine/Math/Math.h"
...
namespace TinyEngine {
	...
	void EditorLayer::OpenScene(const std::filesystem::path& path)
	{
		...
		{
			TE_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
	...
	{
		TE_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
