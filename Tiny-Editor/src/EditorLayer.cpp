...
namespace TinyEngine {
	...
	void EditorLayer::OnUpdate(TinyEngine::Timestep ts)
	{
		TE_PROFILE_FUNCTION();

		if (TinyEngine::FramebufferSpecification spec = m_Framebuffer->GetSpecification();
...

		{
			...
			TinyEngine::Renderer2D::DrawRotatedQuad({ 1.0f, 0.0f }, { 0.8f, 0.8f }, -45.0f, { 0.8f, 0.2f, 0.3f, 1.0f });
			...
			TinyEngine::Renderer2D::DrawQuad({ 0.0f, 0.0f, -0.1f }, { 20.0f, 20.0f }, m_CheckerboardTexture, 10.0f);
			TinyEngine::Renderer2D::DrawRotatedQuad({ -2.0f, 0.0f, 0.0f }, { 1.0f, 1.0f }, rotation, m_CheckerboardTexture, 20.0f);
			...
			for (float y = -5.0f; y < 5.0f; y += 0.5f)
			{
				for (float x = -5.0f; x < 5.0f; x += 0.5f)
				{
					...
					TinyEngine::Renderer2D::DrawQuad({ x, y }, { 0.45f, 0.45f }, color);
				}
			}
			TinyEngine::Renderer2D::EndScene();
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("File"))
			{
				...
				if (ImGui::MenuItem("Exit")) TinyEngine::Application::Get().Close();
				ImGui::EndMenu();
			}
		...
		auto stats = TinyEngine::Renderer2D::GetStats();
	...
	void EditorLayer::OnEvent(TinyEngine::Event& e)
	{
